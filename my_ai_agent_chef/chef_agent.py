"""Head Chef Agent - Plans and coordinates multi-step culinary tasks using Claude."""
...
class ChefAgent(CocinaAgentBase):
    """Head Chef agent that plans and coordinates tasks."""
    ...
    def process_message(self, message):
        ...
        logger.info(f"Chef processing message type: {message_type}")
    ...
    def _handle_delegated_task(self, message, correlation_id, workflow_id):
        ...
        logger.info(f"Chef received task: {task_id}")
        print(f"[Head Chef Agent | ID: {self.agent_id}] Starting task: {task_id}...")
        ...
        try:
            ...
            print(f"[Head Chef Agent | ID: {self.agent_id}] Task {task_id} completed.")
    ...
    def call_claude(self, task_description):
        ...
        print(f"[Head Chef Agent | ID: {self.agent_id}] Result:\n{result}")
        print(f"[Head Chef Agent | ID: {self.agent_id}] Done. (Input tokens: {message.usage.input_tokens}, Output tokens: {message.usage.output_tokens})")
